...
void velocidadeProximaLinha (pixel *linha, pixel *linhaAnterior, int fluxoDesejado, int largura) {
  ...
  for (i = tamanhoDaMargemEsquerda + 1; i < largura-tamanhoDaMargemDireita - 1; i++) {
    if (tipo(&linha[i]) == AGUA && tipo(&linhaAnterior[i]) == AGUA) {
    ...
    else if( tipo(&linha[i]) == AGUA && tipo(&linhaAnterior[i]) == TERRA){
      float aux = sqrt(fluxoDesejado)/largura;
      float novaVelocidade = realRandomico(0.001, aux);
      setaVelocidade( &linha[i], novaVelocidade);
    }
  }
